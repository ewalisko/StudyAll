 ...
 * @param {elementID} elementID - the element in the html that holds 
 * 								  the studyguide
 */
...
StudyGuide.prototype.display = function()
{
	"use strict";
    if(this != null) {
    	var display = (this.description + " - Question " + 
						(this.current+1)).bold() + ": " +
						" (difficulty: " + 
						this.cards[this.current].getDifficulty() 
						+ " category: " + 
						this.cards[this.current].getCategory() + ")" +
    				    this.cards[this.current].getQuestion();
    	if(this.displayHint)
    		display += ("\nHint").bold() + ": " + 
    		this.cards[this.current].getHint();
        if(this.displayAnswer)
        	display += ("\nAnswer").bold() + ": " + 
        	this.cards[this.current].getAnswer();
